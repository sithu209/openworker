"""Prepare Case 0004 immutable visual-review bundle without publishing it.

The bundle is later published by go-tool capability drive.review.publish so the
Google Drive credential comes from the machine-local go-tool credential store.
"""
...
def _collect_artifacts(workspace: Path) -> list[Path]:
    ...
    items.extend(path.resolve() for path in required)
...
def main(argv: list[str] | None = None) -> int:
    ...
    state = {
        "schema_version": "openworker-case0004-visual-bundle/v1",
        "case_id": "0004",
        "review_id": review_id,
        "workspace": str(workspace),
        "bundle": str(bundle),
        "bundle_manifest_sha256": manifest_sha,
        "drive_root_folder_id": str(args.drive_folder_id),
        "artifact_count": len(manifest_items),
        "total_bytes": total_bytes,
        "status": "BUNDLE_READY",
    }
    state_path = workspace / "evidence" / f"case0004-visual-bundle-{run_id}.json"
...
if __name__ == "__main__":
    try:
        ...
        print(f"CASE0004_VISUAL_BUNDLE_PREPARE_FAIL {exc}", file=sys.stderr)
        raise SystemExit(1)
